...
namespace Random_Projectiles_V2 {
    internal static class URandom {
        internal static ulong lastRand = GetNext((ulong)DateTime.Now.ToUniversalTime().TimeOfDay.TotalMilliseconds);

        internal static ulong GetNext(ulong max = 0UL) {
            ...
            return lastRand = ((v0 << 24) + v1) % max == 0UL ? ulong.MaxValue : max;
        }
